(function ( $ )
{
	$(document).ready(function()
	{
		...
		{
			var data = "", encodedData, permalink = false;
			...
			$tallyList.children().each(function()
			{
				...
				if(identifier == "g16.tally.config.permalink")
					permalink = ((count == 1) ? true : false);
			...
			if($tallyList.children().length > 0)
			{
				if(permalink)
				{
					// Send an AJAX GET request to the API backend to generate/update the
					// permalink for the current list
					ajax_callback("GET", "/api/generate.permalink." + encodedData + "." +
							id, function(response)
					{
						console.log("url", "/api/generate.permalink." + encodedData + "." + id);
						return response;
					});
				}
				else
				...
				+ window.location.pathname.substring(1), function(data)
			{
				...
				for(var i = 0; i < fragmentX.length; i++)
				{
					...
					if(fragmentY[0] == "g16.tally.config.name" ||
							...
							fragmentY[0] == "g16.tally.config.permalink")
					{
						...
						// Set the instance ID
						if(fragmentY[0] == "g16.tally.config.permalink")
